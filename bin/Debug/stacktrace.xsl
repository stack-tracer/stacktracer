...
font-size: 13px;
        ...
        <body class="stacktraceinfo" >
          
          <div class="jumbotron">
      <div class="container">
        <h2>Stack Traces for <xsl:value-of select="./StackTracer/processName"/> <xsl:text xml:space="preserve"> </xsl:text>              
          </h2>
        <p>
          Process ID : <b><xsl:value-of select="./StackTracer/processID"/></b>
          
          Total number of samples : <b><xsl:value-of select="count(./StackTracer/sampleCollection/StackSample)"/></b>
          </p>
        <p>
          Date : <b><xsl:value-of select="substring(./StackTracer/sampleCollection/StackSample[1]/samplingTime,0,11)"/></b>
          Start Time: <b><xsl:value-of select="substring(./StackTracer/sampleCollection/StackSample[1]/samplingTime,12,12)"/></b>
      </p>
      </div>
    </div>
          <div id="header">
...
$('#myTable').dataTable();
 /*$("div.timeline-icon").each(function(){$(this).attr("data-hidden","1")});
              $("div.timeline-icon").click(function()
              {
                  ...
                  {
                      $("div.timeline-row"+"[data-osid!="+$(this).attr("data-osid")+"]").hide();
                      $("div.timeline-icon").each(function(){$(this).attr("data-hidden","0")});
                  }
                  else
                  {
                      $("div.timeline-icon").each(function(){$(this).attr("data-hidden","1")});
...
$('#stacksfrmAllSamples').on('show.bs.modal', function (event) {
  ...
  var recipient = button.data("osid"); // Extract info from data-* attributes
...
function nodeClick()
{
 if($(this).attr("data-hidden")=="1")
  ...
  <xsl:template name="StackTraceDetailsAnim">
  ...
  <xsl:template name="StackFrame">
    
    <xsl:value-of select="./clrMethodString"></xsl:value-of> 
    
  </xsl:template>
  <xsl:template name="ThreadStack">
  ...
  <xsl:template name="SampleSummary">
    <tr >      
      ...
      <td>
        <xsl:value-of select="substring(./samplingTime,12,12)"/>
      </td>
  </tr>
  </xsl:template>
<xsl:template name="StackSummary">
  <xsl:for-each select="/StackTracer/sampleCollection/StackSample">
    <div>
      <ul>
        <xsl:for-each select="./processThreadCollection/Thread">
